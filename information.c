...
//M_q, E_q, seq_count, mbins, ebins
//D1表示qv1的维度，2表示只有0和1两种情况
//D2表示qv2的维度，这里表示有多少个基因bins
//qv1和qv2的长度都表示具有表达值的基因bins数
int** ConstructCountMatrix(int* qv1, int* qv2, int m, int D1, int D2) {
    ...
    for (i=0; i<m; i++) {
        //qv1[i]只能是0和1，表示该该motif是否存在于第i个基因bin中
        //qv2[i]只能是0 - (mbins-1)，表示第i基因处于哪一个bins
...
//M_q, E_q, seq_count, mbins, ebins
//D1表示data1的维度，2表示只有0和1两种情况
//D2表示data2的维度，这里表示有多少个基因bins
float CalculateMIbasic(int* data1,  int* data2, int m, int D1, int D2)
{
    ...
    //这里得到的tmpCounts是一个矩阵，是一个两行，D2列矩阵
    tmpCounts   = ConstructCountMatrix (data1, data2, m, D1, D2);
    mi = FindInfoLocalMI(tmpCounts, D1, D2);
    for (i=0; i<D1; i++) {
        free(tmpCounts[i]);
