...
void add_small_values_to_identical_floats(float* E, int n) {
  ...
  for (i=n-2; i>=0; i--) {
    d = fabs(fi[i+1].v - fi[i].v);  //fabs()是求绝对值
    if (d > eps) {
...
/*
 ...
 idea : 10000 * 10 * ( shu + nbkmers) instead of 10000 * 10 * nbkmers * shu

generate a partition of the genes

for all 10
 generate E of size 90%
 for all kmers
  calc mi jn

for all 10000
  for all 10
    gen a shuffled E, of size 90%
    for all kmers
      cross E with M_q
...
//nborfs表示E中元素的个数，E_q只有在quantized==1时才使用
