    ...
    *data     = (float**)malloc(mynmax * sizeof(float*));
    ...
    while (!feof(fp)) {
        ...
        while ((s = mystrtok(0, '\t'))) {
            ...
            if ((strcmp(s, "") != 0) && (strcmp(s, "NA") != 0) && (strcmp(s, "nan") != 0)) {
...
{
    int i = 0;
    //strcmp retures false when two parameters is not equal
...
char* seqI_nextSequence(seqI* s, char** name, int* size)
{
    ...
    if (s->nextSequence_started == 0) {
        //fgets()会读取一行，最多读取len-1个字符，最后一个字符赋值为\0
        ...
        *name = strdup(s->nextSequence_currentLine + 1);
        ...
        while (1) {
            ...
            if ((s->nextSequence_currentLine)[0] == '>') {
                realloc( seq, strlen(seq) + 1);
                //感觉上面这一行应该这么写：
                //seq = (char *)realloc( seq, strlen(seq) + 1);
                return seq;
            } else {
...
char* translate(char* seq, char*** aacode) 
{
    ...
    for (i=0; i<64; i++) {
        //indexOfKmer计算氨基酸序列的索引，这个索引会落在 0~63
    ...
    for (i=0; i<l-rabe-2; i+=3) {
        ...
        free(codon); //substr会拷贝一份给这个指针，所以需要清理内存
    }
...
char *substr(char *string, int start, int length) 
{
    ...
    substring[length] = '\0';  //只有字符指针的最后一位为0，strlen才能计算字符串的长度，长度不包含\0
